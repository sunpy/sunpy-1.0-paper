...
lims_pix_ax1 = map1.world_to_pixel(
    SkyCoord(*lims_arcsec, frame=map1.coordinate_frame))
...
ax1.text(0.03, 0.92, 'a. ' + str(map1.date)
         [0:10], color='w', fontsize=10, transform=ax1.transAxes)
ax2.text(0.03, 0.92, 'b. ' + str(map2.date)
         [0:10], color='w', fontsize=10, transform=ax2.transAxes)
ax3.text(0.03, 0.92, 'c. Rotated map a. to time of b.',
         color='w', fontsize=10, transform=ax3.transAxes)

# define arrow coordinates in pixels
arrows1_pix = map1.world_to_pixel(SkyCoord(
    (-730, -500)*u.arcsec, (450, -400)*u.arcsec, frame=map1.coordinate_frame))
arrows2_pix = map2.world_to_pixel(SkyCoord(
    (-390, -500)*u.arcsec, (780, -400)*u.arcsec, frame=map2.coordinate_frame))
arrows3_pix = rotated_map.world_to_pixel(SkyCoord(
    (-390, -500)*u.arcsec, (780, -400)*u.arcsec, frame=rotated_map.coordinate_frame))

# draw arrows
ax1.arrow(arrows1_pix[0][0].value, arrows1_pix[0]
          [1].value, 0, 180, head_width=30, color='r')
ax1.arrow(arrows1_pix[1][0].value, arrows1_pix[1]
          [1].value, 0, 180, head_width=30, color='w')

ax2.arrow(arrows2_pix[0][0].value, arrows2_pix[0]
          [1].value, 0, 180, head_width=30, color='r')
ax2.arrow(arrows2_pix[1][0].value, arrows2_pix[1]
          [1].value, 0, 180, head_width=30, color='w')

ax3.arrow(arrows3_pix[0][0].value, arrows3_pix[0]
          [1].value, 0, 180, head_width=30, color='r')
ax3.arrow(arrows3_pix[1][0].value, arrows3_pix[1]
          [1].value, 0, 180, head_width=30, color='w')
