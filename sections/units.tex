...
The \code{astropy.units} subpackage provides support for physical quantities through a \code{Quantity} class, which consists of a number and its associated unit(s).
Furthermore, \code{Quantity} objects can be combined in expressions with the unit conversions and cancellations automatically taken into account.
Tests have shown that the performance overhead to using this functionality is typically minimal.

We formally mandate that all user-facing functionality provided by \sunpypkg make use of \code{Quantity} objects unless inappropriate.
